...
`BLACK_DIM_HEX="#181715"` is intentionally hand-tuned rather than calculated with `dim_hex`. A literal dimmed black would still be black, which is mathematically correct but not useful on-screen in a terminal. The warm charcoal value keeps dimmed black visible while preserving the overall palette structure.
